...
if [[ "$1" != "start" && "$1" != "stop" && "$1" != "down" ]]; then
    ...
    if [ "$1" = "start" ]; then
        docker compose -f docker-compose.yml -f docker-compose.deployment.yml -p ubkg-front-end up -d
    elif [ "$1" = "stop" ]; then
        docker compose -f docker-compose.yml -f docker-compose.deployment.yml -p ubkg-front-end stop
    elif [ "$1" = "down" ]; then
        docker compose -f docker-compose.yml -f docker-compose.deployment.yml -p ubkg-front-end down
    fi
fi
