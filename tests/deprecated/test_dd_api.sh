...
Help()
{
   ...
   echo ""
   echo "****************************************"
   echo "HELP: Data Distillery-API test script"
   echo | tee
   echo "Syntax: ./test_api.sh [-option]..."
   echo "option" | tee
   echo "-v     test environment: l (local), d (DEV), or p (PROD)"
   echo "NOTE 1: This script writes output to a file named test.out."
   echo
   echo "NOTE 2: This script assumes that the api key for Data Distillery is stored in a text file named api.key."
}
...
UBKG_URL_PROD=https://datadistillery.api.sennetconsortium.org
UBKG_URL_DEV=https://datadistillery.api.sennetconsortium.org
UBKG_URL_TEST=https://datadistillery.api.sennetconsortium.org
...
echo "DATA DISTILLERY API" | tee test.out
...
curl --request GET \
 --url "${UBKG_URL}/concepts/C0006142/paths/subgraph/sequential?relsequence=NCI%3Ais_marked_by_gene_product&skip=0&limit=-1" \
 --header "Accept: application/json" \
 --header "Authorization: UMLS-Key $umlskey" | tee -a test.out
echo | tee -a test.out
echo | tee -a test.out
...
curl --request GET \
 --url "${UBKG_URL}/concepts/C0006142X/paths/subgraph/sequential?relsequence=NCI%3Ais_marked_by_gene_product,NCI%3Agene_product_encoded_by_gene&skip=0&limit=5" \
 --header "Accept: application/json" \
 --header "Authorization: UMLS-Key $umlskey" | tee -a test.out
echo | tee -a test.out
echo | tee -a test.out
...
curl --request GET \
 --url "${UBKG_URL}/concepts/C0006142/paths/subgraph/sequential?relsequence=NCI%3Ais_marked_by_gene_product,NCI%3Agene_product_encoded_by_gene&skip=0&limit=5" \
 --header "Accept: application/json" \
 --header "Authorization: UMLS-Key $umlskey" | cut -c1-60 | tee -a test.out
echo | tee -a test.out
echo | tee -a test.out
...
curl --request GET \
 --url "${UBKG_URL}/concepts/C0006142/paths/subgraph/sequential?relsequence=NCI%3Ais_marked_by_gene_product&relsequence=NCI%3Agene_product_encoded_by_gene&skip=0&limit=5" \
 --header "Authorization: UMLS-Key $umlskey" | cut -c1-60 | tee -a test.out
echo | tee -a test.out
echo | tee -a test.out
...
echo "TEST FOR: concepts/paths/subgraph/sequential GET" | tee -a test.out
echo "SIGNATURE: /conepts/paths/subgraph/sequential?relsequence=<SAB1:rel1,SAB2:rel2&skip=<skip>&limit=<limit>" | tee -a test.out
echo | tee -a test.out
echo | tee -a test.out

echo "concepts/paths/subgraph/sequential?relsequence=NCI%3Ais_marked_by_gene_product&relsequence=NCI%3Agene_product_encoded_by_gene&skip=0&limit=5 => valid, with individual; should return 200" | tee -a test.out
curl --request GET \
 --url "${UBKG_URL}/concepts/paths/subgraph/sequential?relsequence=NCI%3Ais_marked_by_gene_product&relsequence=NCI%3Agene_product_encoded_by_gene&skip=0&limit=5" \
 --header "Authorization: UMLS-Key $umlskey" | cut -c1-60 | tee -a test.out
echo | tee -a test.out
echo | tee -a test.out
...
#--------------------------------------------
echo "TESTS FOR: semantics/semantic-types/<semantic-type> GET" | tee -a test.out
echo "SIGNATURE: /semantics/semantic-types/<semantic_type>?skip=<number>&limit=<number>" | tee -a test.out
echo | tee -a test.out
echo | tee -a test.out

echo "1. /semantics/semantic-types/Anatomical%20Structure?skip=1&limit=10&test=test => invalid parameter; should return custom 400" | tee -a test.out
curl --request GET \
...
echo "2. /semantics/semantic-types/Anatomical%20Structure?skip=-1&limit=10 => negative skip; should return custom 400" | tee -a test.out
curl --request GET \
...
echo "3. semantics/semantic-types/Anatomical%20Structure?skip=1&limit=-1 => negative limit; should return custom 400" | tee -a test.out
curl --request GET \
...
echo "4. semantics/semantic-types/Anatomical%20Structure?skip=a&limit=10 => non-numeric skip; should return custom 400" | tee -a test.out
curl --request GET \
 --url "${UBKG_URL}/semantics/semantic-types/Anatomical%20Structure?skip=-1&limit=10" \
 --header "Accept: application/json" \
 --header "Authorization: UMLS-Key $umlskey" | tee -a test.out
...
echo "5. semantics/semantic-types/Anatomical%20Structure?skip=1&limit=a => non-numeric limit; should return custom 400" | tee -a test.out
curl --request GET \
 --url "${UBKG_URL}/semantics/semantic-types/Anatomical%20Structure?skip=1&limit=-1" \
 --header "Accept: application/json" \
 --header "Authorization: UMLS-Key $umlskey" | tee -a test.out
...
echo "6. semantics/semantic-types/Anatomical%20Structurez?skip=1&limit=10 => no match; should return custom 404" | tee -a test.out
curl --request GET \
